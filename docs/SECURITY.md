...
Self-service signup is opt-in. Verified accounts without membership may only use personal account/onboarding APIs; workspace endpoints still require current membership. New organizations are isolated, start without approved targets and receive only their creator as owner. Company names or matching email domains do not confer membership. Account and organization quotas are enforced transactionally; add edge abuse controls for a public launch.

Invitations are bound to the exact verified email, expire in seven days, and require explicit acceptance. The shared `/join` URL contains no bearer credential and grants no access itself. Acceptance consumes the invitation in the same transaction as membership creation, checks current inviter ownership and project scopes, and rejects existing memberships. Owners can revoke pending invitations immediately. Expired invitations are excluded from results and removed on subsequent invitation creation; audit records remain.

Sessions expire after 12 hours or 30 idle minutes. Routine polling does not count as activity. The account portal can revoke all other sessions for the current identity while retaining its active session. Password, MFA, email verification and account recovery are delegated to the configured identity provider. Self-service account deletion, automated offboarding and email delivery are not implemented; operators must handle identity/audit data lifecycle under their retention policy. Removing a membership blocks new requests for that organization, including a still-open session. Identity-provider logout and deactivation alone do not immediately invalidate existing inspector sessions. MFA is a provider policy; the optional required ACR validates a provider-defined authentication context.
